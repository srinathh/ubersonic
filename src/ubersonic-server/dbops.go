...
func (sdb *SubsonicDB) Open() (err error) {
	...
	db, err = sql.Open("ql", fmt.Sprintf("file://%s?mode=ro", sdb.dbpath))
	if err != nil {
		return
	}
...
func (sdb *SubsonicDB) GetIndexedArtists() (indexesptr *[]*SubsonicIndex, err error) {
	...
	rows, err = sdb.statements[GETARTISTS].Query()
	if err != nil {
		return
	}
...
func (sdb *SubsonicDB) GetCoverArt(id string) (coverArt []byte, err error) {
	...
	switch id[0:3] {
	case "ar-":
		rows, err = sdb.statements[GETARTISTART].Query(id[3:])
		if err != nil {
			return
		}
	case "al-":
		rows, err = sdb.statements[GETALBUMART].Query(id[3:])
		if err != nil {
			return
		}
	default:
...
func (sdb *SubsonicDB) GetArtist(id string) (sr *SubsonicArtist, err error) {
	...
	rows, err = sdb.statements[GETARTISTALBUMS].Query(id)
	if err != nil {
		return
	}
...
func (sdb *SubsonicDB) GetAlbum(id string) (sr *SubsonicAlbum, err error) {
	...
	rows, err = sdb.statements[GETALBUMSONGS].Query(sr.ArtistId, sr.Id)
	if err != nil {
		return
	}
...
func (sdb *SubsonicDB) CheckPassword(username string, password string) (ok bool, err error) {
	...
	err = sdb.statements[GETPASSWORD].QueryRow(username, password).Scan(&count)
	if err != nil {
		return
	}
...
func (sdb *SubsonicDB) scanAlbum(row *sql.Rows) (sa *SubsonicAlbum, err error) {
	...
	err = row.Scan(&id, &title, &artistid, &artist)
	if err != nil {
		return
	}
